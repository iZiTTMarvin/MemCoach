        ...
        <div className="flex-1 overflow-y-auto p-2">
          {topicKeys.map((key) => (
            <div key={key} className="relative mb-1 group">
              <button
                className={`w-full px-3 py-2.5 text-xs text-left cursor-pointer flex items-center gap-3 transition-all border border-transparent
      ...
      {showAddTopic && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowAddTopic(false)}>
          <div className="bg-bg-subtle border border-primary/30 p-6 w-[400px] max-w-full relative shadow-[0_0_40px_rgba(16,185,129,0.1)]" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-center gap-2 text-primary font-bold text-sm tracking-widest mb-6 uppercase border-b border-primary/20 pb-3">
              <Terminal size={16} /> 分配新领域
            </div>
            
            <div className="mb-4">
              <label className="text-[11px] text-dim mb-1.5 block uppercase tracking-wider">系统标识 (英文)</label>
              <input className="w-full px-3 py-2 border border-border bg-bg text-text text-sm focus:border-primary outline-none transition-colors" placeholder="如: docker" value={newTopicKey} onChange={(e) => setNewTopicKey(e.target.value)} autoFocus />
            </div>
            <div className="mb-4">
              <label className="text-[11px] text-dim mb-1.5 block uppercase tracking-wider">显示名称</label>
              <input className="w-full px-3 py-2 border border-border bg-bg text-text text-sm focus:border-primary outline-none transition-colors" placeholder="如: Docker 容器化" value={newTopicName} onChange={(e) => setNewTopicName(e.target.value)} />
            </div>
            <div className="mb-6">
              <label className="text-[11px] text-dim mb-2 block uppercase tracking-wider">领域图标</label>
            ...
            <div className="flex gap-3 justify-end mt-8 border-t border-primary/10 pt-4">
              <button className="px-5 py-2 border border-border bg-card text-dim text-xs tracking-wider uppercase hover:text-text transition-colors" onClick={() => { setShowAddTopic(false); setNewTopicKey(""); setNewTopicName(""); setNewTopicIcon("FileText"); }}>取消</button>
              <button className="px-5 py-2 bg-primary/20 border border-primary text-primary text-xs font-bold tracking-wider uppercase hover:bg-primary hover:text-bg transition-colors disabled:opacity-30 disabled:hover:bg-primary/20 disabled:hover:text-primary" onClick={handleAddTopic} disabled={!newTopicKey.trim() || !newTopicName.trim()}>执行添加</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col relative z-10 overflow-hidden">
        ...
        <div className="bg-bg-subtle/80 backdrop-blur-md border-b border-primary/20 pt-4 px-6 shrink-0">
          <div className="flex items-center justify-between mb-4">
            <h1 className="text-xl font-display font-bold uppercase tracking-widest text-primary flex items-center gap-2">
              <Activity size={18} />
              知识矩阵矩阵
            </h1>
            {selected && (
              <div className="text-xs text-dim bg-primary/5 border border-primary/20 px-3 py-1 flex items-center gap-2">
          ...
          <div className="flex gap-4">
            <button
              className={`px-4 py-2 text-xs uppercase tracking-wider transition-all border-b-2 ${tab === "core" ? "text-primary border-primary bg-primary/5" : "text-dim border-transparent hover:text-text hover:bg-card"}`}
              onClick={() => setTab("core")}
            >核心数据</button>
            <button
              className={`px-4 py-2 text-xs uppercase tracking-wider transition-all border-b-2 ${tab === "high_freq" ? "text-accent border-accent bg-accent/5" : "text-dim border-transparent hover:text-text hover:bg-card"}`}
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 custom-scrollbar">
          {!selected ? (
            ...
            <div className="max-w-5xl mx-auto w-full">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4 border border-border/50 bg-card/40 p-4">
                ...
                <div className="flex flex-wrap items-center gap-2 shrink-0">
                  {showNewFile ? (
                    <div className="flex gap-2 w-full sm:w-auto">
                      <input className="flex-1 sm:w-48 px-3 py-1.5 border border-primary/30 bg-bg text-primary text-xs focus:border-primary outline-none" placeholder="文件名.md" value={newFileName} onChange={(e) => setNewFileName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleCreateFile()} autoFocus />
                      <button className="px-4 py-1.5 bg-primary/20 border border-primary text-primary text-xs uppercase hover:bg-primary hover:text-bg transition-colors" onClick={handleCreateFile}>初始化</button>
                      <button className="px-3 py-1.5 border border-border bg-card text-dim text-xs uppercase hover:text-text transition-colors" onClick={() => { setShowNewFile(false); setNewFileName(""); }}><X size={14}/></button>
                    </div>
                  ) : (
                    <>
                      <button className="px-4 py-2 border border-border bg-card hover:border-primary/50 text-text text-xs tracking-wider uppercase transition-colors flex items-center gap-1.5" onClick={() => setShowNewFile(true)}>
                        <FileCode size={14} className="text-primary" /> 分配新文件
                      </button>
                      {coreIsEmpty && (
                        <button
                          className="px-4 py-2 bg-accent/10 border border-accent/40 text-accent text-xs tracking-wider uppercase hover:bg-accent hover:text-bg transition-colors disabled:opacity-50 flex items-center gap-1.5"
                ...
                <div className="flex flex-col gap-4">
                  {coreFiles.map((f) => (
                    <div key={f.filename} className="bg-bg-subtle border border-primary/20 overflow-hidden relative group">
                      ...
                      <div
                        className="flex justify-between items-center px-5 py-3 cursor-pointer text-sm font-semibold tracking-wider hover:bg-primary/5 transition-colors border-b border-transparent group-hover:border-primary/10"
                        onClick={() => setExpandedFile(expandedFile === f.filename ? null : f.filename)}
                      >
                        <span className="flex items-center gap-2 text-text">
                          <FileCode size={16} className="text-primary" /> {f.filename}
                        </span>
                        <div className="flex items-center gap-4">
                          <span className="text-[10px] text-dim font-mono">{f.content?.length || 0} 字节</span>
                          <div className="flex items-center gap-2">
                            <button
                              className="text-dim hover:text-red-400 transition-colors px-1 py-1"
                              title="Delete File"
                              onClick={(e) => { e.stopPropagation(); handleDeleteFile(f.filename); }}
                            ><X size={16} /></button>
                            <span className="text-primary">{expandedFile === f.filename ? <ChevronDown size={18} /> : <ChevronRight size={18} />}</span>
                          </div>
                        </div>
                      </div>
                      
                      {expandedFile === f.filename && (
                        <div className="border-t border-primary/20 bg-bg p-4 relative">
                          <textarea
                            className="w-full min-h-[400px] p-4 bg-bg border border-border text-primary/90 text-[13px] font-mono leading-relaxed resize-y focus:border-primary focus:ring-1 focus:ring-primary/20 outline-none custom-scrollbar"
                            value={editContent[f.filename] ?? f.content}
                            onChange={(e) => setEditContent((prev) => ({ ...prev, [f.filename]: e.target.value }))}
                            spellCheck={false}
                          />
                          <div className="flex items-center justify-end gap-3 mt-4">
                            {coreSaving === f.filename && <span className="text-xs text-accent uppercase tracking-widest flex items-center gap-1"><Sparkles size={12}/> 数据已保存</span>}
                            <button className="px-6 py-2 bg-primary border border-primary text-bg text-xs font-bold uppercase tracking-wider hover:bg-primary-hover hover:border-primary-hover transition-colors shadow-[0_0_15px_rgba(16,185,129,0.2)]" onClick={() => handleSaveCore(f.filename)}>编译并保存</button>
                          </div>
                        </div>
                      )}
                    </div>
            ...
            <div className="max-w-5xl mx-auto w-full flex flex-col h-full">
              <div className="text-xs text-dim mb-4 p-4 border border-accent/20 bg-accent/5 text-accent-light font-sans">
                高优先级抗压测试节点。系统将更加积极地从该列表采样。支持 Markdown。
              </div>
              <div className="flex-1 relative min-h-[500px]">
                <textarea
                  className="absolute inset-0 w-full h-full p-5 bg-bg-subtle border border-border text-accent/90 text-[13px] font-mono leading-relaxed resize-none focus:border-accent focus:ring-1 focus:ring-accent/20 outline-none custom-scrollbar"
                  value={highFreqDraft}
              ...
              <div className="flex items-center justify-end gap-3 mt-4 shrink-0">
                {hfSaving && <span className="text-xs text-accent uppercase tracking-widest flex items-center gap-1"><Sparkles size={12}/> 数据库已同步</span>}
                {highFreqDraft !== highFreq && (
                  <button className="px-5 py-2 border border-border text-dim text-xs uppercase hover:text-text transition-colors" onClick={() => setHighFreqDraft(highFreq)}>还原更改</button>
                )}
                <button className="px-6 py-2 bg-accent/20 border border-accent text-accent text-xs font-bold uppercase tracking-wider hover:bg-accent hover:text-bg transition-colors disabled:opacity-30 disabled:hover:bg-accent/20 disabled:hover:text-accent" onClick={handleSaveHighFreq} disabled={highFreqDraft === highFreq}>更新节点</button>
              </div>
            </div>
          )}
        </div>
