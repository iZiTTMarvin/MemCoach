...
**临时文件类（严格禁止）：**
- 在项目根目录下创建临时文件/目录（会触发 uvicorn --reload 导致服务重启）
- 临时文件必须使用 `tempfile.mkdtemp()` 写入系统 temp 目录
- 仅 `data/` 目录允许持久化写入，其余项目内目录禁止运行时写入
...
- [ ] 临时文件使用 `tempfile.mkdtemp()`，未在项目根目录下创建写入型目录
